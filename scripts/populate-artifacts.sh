...
# Platform-specific system libraries (not in pkg-config)
...
# Uses pkg-config to query actual .pc files - fails if unavailable.
# Arguments:
#   $1 - Output file path
...
generate_link_flags_js() {
  ...
  # Require pkg-config directory
  if [[ ! -d "${pc_dir}" ]]; then
    log_error "pkg-config directory not found: ${pc_dir}"
    return 1
  fi

  # Query flags from pkg-config (single source of truth)
  local flags
  flags=$(query_link_flags_from_pkgconfig "${pc_dir}" "${platform}")

  if [[ -z "${flags}" ]]; then
    log_error "pkg-config query failed for ${platform}"
    log_error "Ensure pkg-config is installed and .pc files exist in ${pc_dir}"
    return 1
  fi

  log_info "  Generated link flags from pkg-config"
...
populate_platform() {
  ...
  # Generate link flags from pkg-config (fails if pkg-config unavailable)
  if ! generate_link_flags_js "${npm_dest}/link-flags.js" "${platform}" "${tier}"; then
    log_error "Failed to generate link-flags.js for ${npm_dir_name}"
    return 1
  fi
