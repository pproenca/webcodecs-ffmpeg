#!/usr/bin/env bash
#
# populate-npm.sh - Populate npm packages from build artifacts (DEPRECATED)
#
# DEPRECATED: This script regenerates package.json files which overwrites
# committed versions. For releases, use:
#   - bump-version.sh: Updates versions in package.json files
#   - populate-artifacts.sh: Copies build artifacts only
#
# This script is kept for local development/testing where you need to
# regenerate everything from scratch.
#
# Usage: ./scripts/populate-npm.sh
#
# Reads from: artifacts/<platform>-<tier>/
# Writes to:  npm/<platform>[-non-free]/
...
generate_platform_package_json() {
  ...
  local npm_subdir="webcodecs-ffmpeg-${platform}"
  if [[ "${tier}" != "free" ]]; then
    pkg_name="${pkg_name}-${tier}"
    npm_subdir="webcodecs-ffmpeg-${platform}-${tier}"
  fi
...
generate_meta_package_json() {
  ...
  local npm_subdir="webcodecs-ffmpeg"
  if [[ "${tier}" != "free" ]]; then
    pkg_name="${pkg_name}-${tier}"
    npm_subdir="webcodecs-ffmpeg-${tier}"
  fi
...
function getPackageName(platformKey) {
  return `@pproenca/webcodecs-ffmpeg-${platformKey}${TIER_SUFFIX}`;
}
...
main() {
  ...
  # Regenerate root package.json (workspaces defined in pnpm-workspace.yaml)
