...
FFmpeg prebuilds - a modular build system for creating statically-linked FFmpeg binaries with codec dependencies for multiple platforms. Distributed as npm packages (`@pproenca/webcodecs-ffmpeg`, `@pproenca/webcodecs-ffmpeg-non-free`) for use in Node.js native addons.

**Goals:**
- Provide ready-to-use static FFmpeg libraries for native addon development
- Support multiple platforms (macOS ARM64/x64, Linux ARM64/x64)
- Offer two license tiers (free, non-free) with different codec sets
...
**Distribution:**
- pnpm workspaces (internal package management)
- npm registry (published packages for consumers)

**Development Tools:**
- mise (task runner, tool versioning)
- pnpm 9 (workspace management, publishing)
...
└── codecs/                # Centralized codec recipes
    ├── bsd/               # BSD-licensed (libvpx, aom, dav1d, svt-av1, opus, ogg, vorbis)
    ├── lgpl/              # LGPL-licensed (lame)
    └── gpl/               # GPL-licensed (x264, x265)

npm/                       # pnpm workspace for npm packages
├── pnpm-workspace.yaml    # Workspace configuration
├── webcodecs-ffmpeg/      # @pproenca/webcodecs-ffmpeg (free tier)
└── webcodecs-ffmpeg-non-free/  # @pproenca/webcodecs-ffmpeg-non-free (GPL tier)
```

**Build Patterns:**
- Stamp files for dependency tracking and incremental builds
- Platform includes shared codecs; only `config.mk` differs
- License tiers: `LICENSE=free|non-free` controls which codecs build

**CI/CD Architecture:**
- `_build.yml`: Reusable build workflow (matrix: 4 platforms × 2 licenses = 8 jobs)
...
2. Run `./scripts/bump-version.sh patch|minor|major` locally
3. Push commit and tag: `git push origin master v0.7.0`
4. Manual dispatch of `release.yml` with tag name
5. Workflow reuses CI artifacts, creates GitHub Release, publishes to npm

**Version Management:**
- Source of truth: `npm/webcodecs-ffmpeg/package.json`
- `bump-version.sh`: Updates all package.json files, commits, creates tag
- `populate-artifacts.sh`: Copies build artifacts (lib/, include/) to npm packages

## Domain Context

**FFmpeg License Tiers:**
| Tier | License | Codecs | FFmpeg Flag | Use Case |
|------|---------|--------|-------------|----------|
| free | LGPL-2.1+ | libvpx, aom, dav1d, svt-av1, opus, ogg, vorbis, lame | (default) | Commercial/proprietary apps (LGPL-safe) |
| non-free | GPL-2.0+ | All above + x264, x265 (H.264/H.265) | `--enable-gpl` | Open source projects (GPL-licensed) |
...
**CI Services:**
- GitHub Actions (build runners: macos-14 ARM, macos-13 Intel, ubuntu-latest for Linux)
